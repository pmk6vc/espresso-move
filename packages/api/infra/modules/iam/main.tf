...
/**
* Assign the role required to manage service account IAM policies for applying Terraform changes
* Needs to be applied before this service account can be used to manage cloud resources during CI/CD
*/
...
module "oidc" {
  ...
  pool_id = "managed-pool"
  provider_id = "managed-pool-gh-provider"
  sa_mapping = {
    (google_service_account.service_account.account_id) = {
      sa_name = google_service_account.service_account.name
      attribute = "attribute.repository/pmk6vc/espresso-move"
    }
  }
}